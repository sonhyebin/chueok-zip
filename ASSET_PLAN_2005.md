...
- visual_direction: 노래방에서 마이크를 잡고 고개 젖혀 열창하는 학생의 실루엣/뒷모습, 미러볼 역광
- what_should_be_visible: 마이크 쥔 손과 실루엣, 노래방 조명 — 얼굴 비노출
...
- recommended_asset_type: **actual (자체 촬영)** — 없으면 generated
- why: 실물 기기(아이리버 프리즘형 등)는 세대 기억이 형태에 정확히 붙어 있어 실물감이 중요. 중고 실물을 구해 직접 촬영하면 라이선스가 완전히 깨끗함(단순 제품 형태의 촬영은 안전, 로고는 최소 노출).
...
- source_candidate: 중고 실물 직접 촬영 / Wikimedia Commons의 CC0·CC-BY 기기 사진(라이선스 개별 확인 필수)
- license_note: CC-BY 사용 시 저작자 표기 필요 — 서비스 크레딧 페이지 필요해짐. 자체 촬영이 가장 깔끔
- fallback_plan: generated — 무(無)브랜드 프리즘형 MP3를 쥔 손
...
- why: 실제 얼짱 사진·일반인 셀카는 초상권 문제로 절대 불가. "45도 위에서 내려찍는 각도" 자체가 기억 포인트이므로 각도와 포즈만 재현하면 됨.
- visual_direction: 팔을 위로 뻗어 디카/폰을 45도로 내려 든 구도 — **카메라를 든 손과 기기 중심**, 얼굴은 프레임 밖이거나 플래시로 하얗게 날아간 처리
- what_should_be_visible: 위로 뻗은 팔, 디카 뒷면, 과노출 플래시 빛
- crop_recommendation: 대각선 구도, 화면 절반이 플래시 화이트아웃
- aspect_ratio_recommendation: 4:3
- copyright_note: 실존 인물(얼짱 세대 유명인 포함)과 닮은 얼굴 생성 금지 — 얼굴 비노출 구도로 원천 회피
- fallback_plan: 거울 앞 디카 셀카 — 플래시 반사로 얼굴이 완전히 가려진 구도
...
- recommended_asset_type: **actual (자체 촬영)** — generated도 충분
- why: 음식은 실물 촬영이 가장 침샘·기억 자극이 강하고, 직접 만들어 찍으면 권리 이슈 제로. 분식집 협조 촬영도 용이.
- visual_direction: 종이컵에 담긴 떡볶이 + 꽂힌 이쑤시개(나무), 학교 앞 분식집 느낌의 스테인리스 판매대, 김 서림
- what_should_be_visible: 종이컵, 이쑤시개, 빨간 양념의 광택
- crop_recommendation: 컵 클로즈업 로우앵글, 배경 보케
- aspect_ratio_recommendation: 4:3
- copyright_note: 프랜차이즈 로고 컵 금지 — 무지 종이컵
- fallback_plan: generated — 동일 구도 (음식 생성 품질 높음)
...
| generated | 6 | device-1, photo-1·2, fashion-1·2, food-2 |
| actual (자체 촬영 우선) | 2 | internet-2(MP3), food-1(컵떡볶이) |
...
> 상업 확장을 전제로, 외부 다운로드 이미지는 원칙적으로 쓰지 않고 **생성 + 자체 촬영** 2트랙으로 간다.
...
- 권장 스펙: **800×600 (4:3), JPG 품질 80, 200KB 이하**, 우하단 10% 여백(날짜 스탬프 영역)
...
5. **2005-food-1 컵떡볶이** (actual 자체 촬영) — 제작 난이도가 가장 낮으면서 공감 강도가 높음. 타임캡슐 질문("학교 끝나고 가장 많이 갔던 곳")과 감정선이 직결되어 공유 동기를 보강.
