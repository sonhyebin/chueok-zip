...
import { channelFromParam } from "@/lib/channels";

/**
 * 페이지 방문을 1회 기록한다 (link_visit).
 *
 * page: 어떤 화면인지 라벨 ("home" | "timeline" | "result" | "year-2004" 등).
 * 공유·배포 링크로 들어오면 s= 채널 태그(lib/channels.ts 의 코드표: k=카카오, th=스레드, et=에브리타임 …)와
 * memory= 카드 id가 함께 잡힌다. 모르는 코드도 그대로 채널명으로 기록된다.
 *
 * 집계 단위(card):
...
export default function VisitTracker({ page }: { page: string }) {
  useEffect(() => {
    ...
    const src = channelFromParam(params.get("s"));
